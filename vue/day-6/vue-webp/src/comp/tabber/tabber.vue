<template>
    ...
    <a v-for="(item,index) in taber" :key="index" @click="heightLheight(index)" href="javascript:;" v-bind:class="['weui-tabbar__item',
...
<!-- v-for="(item,index) in 数组"：遍历数组（each） -->
<!-- v-if():有或者没有：增加（addend）或者删除（remove） -->
<!-- v-text():拿到数组里的某一个值 -->
<!-- v-on(简写 @ )：点击，事件绑定 -->
<!-- v-bind(简写 ：)：布尔值，更改属性值 -->
</template>
